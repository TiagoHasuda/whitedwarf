...
async function buildAndWriteDeployFile() {
  ...
  try {
    await execute(\`aws apigateway get-method --rest-api-id=${process.env.AWS_REST_API_ID} --resource-id=\${currPath.id} --http-method=\${func.httpMethod}\`)
    methodExists = true
  } catch {}
  if (methodExists) {
  ...
  try {
    await execute(\`aws apigateway get-integration --rest-api-id=${process.env.AWS_REST_API_ID} --resource-id=\${currPath.id} --http-method=\${func.httpMethod}\`)
  ...
  const resourcesToExclude = currResourcesRaw.filter(currResourceItem => !paths.include(currResourceItem.path))\n
