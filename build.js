...
async function buildAndWriteDeployFile() {
  ...
  try {
    await execute(\`aws apigateway put-integration --rest-api-id=${process.env.AWS_REST_API_ID} --resource-id=\${currPath.id} --http-method=POST --type=AWS_PROXY --integration-http-method=\${func.httpMethod} --content-handling=CONVERT_TO_TEXT --passthrough-behavior=WHEN_NO_MATCH --timeout-in-millis=10000 --uri=\${uri} \`)
